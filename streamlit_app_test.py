    ...
    with st.spinner("⏳ テーブル定義を取得中..."):
            ...
            if not df_def_all.empty:
                ...
                if export_mode == "Excelとしてダウンロード（選択テーブルのみ）":
                    sel_db = st.selectbox("データベースを選択", active_dbs)
                    schemas_in_db = [s for (d, s) in active_schemas if d == sel_db]
                    sel_schema = st.selectbox("スキーマを選択", schemas_in_db)

                    tables_in_schema = [t.split(".")[2] for t in active_tables if t.startswith(f"{sel_db}.{sel_schema}.")]
                    sel_table = st.selectbox("テーブルを選択", tables_in_schema)
                    ...
                    st.download_button(
                        ...
                        file_name=f"{sel_db}_{sel_schema}_{sel_table}_definition.xlsx"
                    )
                else:
